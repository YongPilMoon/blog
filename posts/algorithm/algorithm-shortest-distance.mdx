---
title: '[알고리즘] 게임 맵 최단거리'
date: '2022-02-10'
description: 'bfs'
published:  true
---

[문제링크](https://school.programmers.co.kr/learn/courses/30/lessons/1844?language=javascript#)
<br/>
### 문제 설명
문제 설명
...
function solution(maps) {
    ...
    while(queue.length) {
        ...
        if(y === rowLength - 1 && x === colLength - 1) return distance
            
        for(let i=0; i<4; i++) {
            const ny = y + directions[i][0];
            const nx = x + directions[i][1];
            if(ny >=0 && ny < rowLength && nx >= 0 && nx < colLength) {
                if(maps[ny][nx] === 1) {
                    maps[ny][nx] = 0;
...
<br/>
- bfs 활용
- 방문해야할 칸을 queue에 `push`하고 방문 시 `shift`를 사용해서 queue에서 꺼낸다.
- 방문할 칸이 맵을 벗어나지 않는지 체크한다.
- 방문한 값은 재 방문하지 않도록 0으로 변경해준다.
- 2차원 배열에서 첫번째 값은 열(y)임을 주의한다.